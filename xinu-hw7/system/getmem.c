...
void *getmem(ulong nbytes)
{
    ...
    while (curr != NULL) { //loops through the whole freelist until it finds a place to put new memory
        if (curr -> length < nbytes) { //not enough space, keeps looping to find spot where there is enough spcae
            ...
            if (prev == NULL) //if the memory block is the first in the list, sets the freelist.head to leftover, keeping prev as null
                freelist[cpuid].head = leftover;
            else  //otherwise, puts leftover after the previous block
                prev -> next = leftover; 

            if (left <= 0) //if there isnt any space left in the current block, moves onto the next block for the next loop 
                leftover = curr -> next;

            freelist[cpuid].length = free; //leftover overall memory 
